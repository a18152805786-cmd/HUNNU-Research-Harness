...
class CNKIParserTests(unittest.TestCase):
    ...
    def test_a_search_without_a_space_or_a_title_hyphen_or_plus_sends_the_same_url_as_before(self) -> None:
        # If this fails, the change reached past spaces, title hyphens and title "+" signs, and
        # searches that already worked -- exact titles, author names and keywords alike -- now
        # send CNKI a different request.  (A hyphen or a "+" in a title search is the other
        # deliberate change; the HIF-1α and 互联网+ titles that used to sit here never found
        # their papers, see the hyphen and plus tests.)
        cases = (
            ("keyword", "SU", "城市轨道交通"),
