...
- `ListingOnly: true` turns a restricted search into a candidate list: each confirmed row is kept as the page states it (title, journal, year, authors) and no article is opened -- no exact-title relock search, no article page, no access check -- so a 15-row journal search is one page load instead of about thirty. It requires a restriction and `MaxDownloads: 0`; anything else is an `INVALID_REQUEST`. Its records are not identity-confirmed and can never become download candidates; acquire a chosen one afterwards by exact title (`acquire --title`, or an `acquire-batch` item), which opens, locks and checks it then. `acquire` reports `"ListingOnly": true` under `SearchRestriction`, and the query log shows `ResultsInspected` 0.
